---
layout: page
---

<HeroSection />

<div class="home-section">
  <div class="card">
    <h2>One site, four rungs of conformance</h2>
    <p class="sub">Start at Level 1, climb as far as you need. Partial conformance is valid to ship.</p>
    <ConformanceLadder />
    <p class="workflow-cta"><a class="link-cta" href="/quickstart">Start at Level 1 <span class="arr" aria-hidden="true">→</span></a></p>
  </div>
</div>

<div class="home-section">
  <div class="sec-head">
    <h2>Two interfaces, one site</h2>
    <p class="sub">The web has spent thirty years optimizing one of them.</p>
  </div>
  <div class="two-grid">
    <div class="panel human">
      <div class="panel__head"><span class="panel__mark" aria-hidden="true"></span><h3>Human visitor</h3></div>
      <ul>
        <li><b>Needs</b> visual hierarchy, images, interaction</li>
        <li><b>Interface</b> HTML + CSS + JavaScript</li>
        <li><b>Cost</b> bandwidth — cheap</li>
      </ul>
      <p class="panel-quote">The web has spent 30 years optimizing this interface.</p>
    </div>
    <div class="panel agent">
      <div class="panel__head"><span class="panel__mark" aria-hidden="true"></span><h3>LLM agent</h3></div>
      <ul>
        <li><b>Needs</b> structured facts, relationships, freshness</li>
        <li><b>Interface</b> Agent View — structured, queryable</li>
        <li><b>Cost</b> tokens — expensive</li>
      </ul>
      <p class="panel-quote">index-ai builds the interface the agent side never had.</p>
    </div>
  </div>
  <figure class="home-fig">
    <img src="/images/index_ai_visual_metaphor.jpg" alt="index-ai visual metaphor — one site, two interfaces: the human page and the agent view" width="1376" height="768" loading="lazy" decoding="async">
  </figure>
</div>
